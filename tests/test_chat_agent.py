...
def test_generate_country_brief_produces_pdf_for_valid_country():
    ...
    assert file_record["bytes"] == b"%PDF-fake"
    assert "Kenya" in confirmation
...
def test_generate_country_brief_unrecognized_country_returns_no_file():
    confirmation, file_record = _generate_country_brief(FIXTURE_DF, {"country": "Wakanda"})
    assert file_record is None
    assert "not a recognized country" in confirmation


def test_run_chat_turn_calls_country_brief_tool():
    client = _FakeClient([
        _FakeResponse(
            [_FakeToolUseBlock("generate_country_intelligence_brief", {"country": "Kenya"})],
            stop_reason="tool_use",
        ),
