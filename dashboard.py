...
# main dashboard tabs.
# Login gate REMOVED (Chris, 2026-07-08): the Supabase auth flow was more
# friction than value ("it wasn't working well for me and I want to revert
# back to the open form") and runaway-cost worry is handled by the monthly
# spend limit on his Anthropic console account instead. The chat keeps its
# per-browser-session message cap (MAX_TURNS_PER_SESSION) as the abuse
# stopgap. scripts/lib/supabase_auth.py and supabase_setup.sql stay in the
# repo (tested, working against GoTrue) in case member auth returns -- only
# the dashboard wiring was removed.
# Anonymous visit counter -- one Supabase insert per browser session (not
# per rerun; Streamlit reruns this whole script on every interaction, so
# the session_state guard is what keeps this to one row per visitor
# session). Timestamp only, no personal data. No-op until the site_visits
# table exists (supabase_setup.sql).
if not st.session_state.get("visit_logged"):
    record_visit(get_auth_config())
